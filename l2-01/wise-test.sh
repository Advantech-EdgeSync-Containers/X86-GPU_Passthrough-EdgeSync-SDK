...
check_cudnn() {
    ...
    CUDNN_H_PATH=$(find /usr -name cudnn.h 2>/dev/null | head -1)
...
EOL
    ...
    if [ $NVIDIA_STATUS -eq 0 ] && [ $CUDA_STATUS -eq 0 ] && [ $CUDNN_STATUS -eq 0 ] && [ $TEST_STATUS -eq 0 ] && [ $COMP_STATUS -eq 0 ]; then
        OVERALL="PASS"
        SCORE_BAR="[##########] 100%"
        STATUS_ICON="✅"
...
EOL
    ...
    echo -e "Created and maintained by: samir.singh@advantech.com and apoorv.saxena@advantech.com"
